...
class scDEC(object):
    def __init__(self, g_net, h_net, dx_net, dy_net, x_sampler, y_sampler, nb_classes, data, pool, batch_size, alpha, beta, is_train):
        ...
        #self.corr_loss = 1 - self.get_corr(self.y__)
        ...
        #self.corr_optim = tf.train.AdamOptimizer(learning_rate=self.lr, beta1=0.5, beta2=0.9) \
        #        .minimize(self.corr_loss, var_list=self.g_net.vars+self.h_net.vars)
        ...
        #self.corr_summary = tf.summary.scalar('corr_loss',self.corr_loss)
        self.corr_summary = tf.summary.scalar('lr',self.lr)
        self.g_merged_summary = tf.summary.merge([self.g_loss_adv_summary, self.h_loss_adv_summary,\
            self.l2_loss_x_summary,self.l2_loss_y_summary,self.gpx_loss_summary,self.gpy_loss_summary,self.corr_summary])
    ...
    def train(self, nb_batches):
        data_y = self.y_sampler.load_all()[0]
        ...
        for batch_idx in range(nb_batches):
            ...
            for _ in range(5):
                ...
                by = self.y_sampler.get_batch(self.batch_size)
            ...
            #correlation
            #_ = self.sess.run(self.corr_optim, feed_dict={self.x: bx, self.x_onehot: bx_onehot, self.y: by, self.lr:lr})
            ...
            if batch_idx % batches_per_eval == 0:
                ...
                #print(self.sess.run(self.corr_loss, feed_dict={self.x: bx, self.x_onehot: bx_onehot, self.y: by, self.lr:lr}))
    ...
    def get_corr(self, y__):
        ...
        rna_rec = y__[:,:int(self.y_dim/2)]
        atac_rec = y__[:,int(self.y_dim/2):]
        ...
        sim = self.get_cosine_similarity(rna_select_rec_org,atac_select_rec_org)
        print(rna_rec_org,atac_rec_org,rna_select_rec_org,atac_select_rec_org)
        print(sim)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', type=str, default='kidney',help='name of dataset')
    ...
    h_net = model.Encoder_cluster(input_dim=y_dim,output_dim = x_dim+nb_classes,feat_dim=x_dim,name='h_net',nb_layers=10,nb_units=256)
    ...
    ys = util.scAI_Sampler(n_components=10,mode=mode)

    #gene_idx, re_idx_list = util.find_RE_index(ys.genes, ys.peaks)
    import hickle as hkl
    gene_idx, re_idx_list = hkl.load('index.hkl')
    rna_mean = ys.rna_reducer.mean_.astype('float32')
    rna_comp = ys.rna_reducer.components_.astype('float32')
    atac_mean = ys.atac_reducer.mean_.astype('float32')
    atac_comp = ys.atac_reducer.components_.astype('float32')
    #rna_mean = tf.convert_to_tensor(ys.rna_reducer.mean_,dtype=tf.float32)
    #rna_comp = tf.convert_to_tensor(ys.rna_reducer.components_,dtype=tf.float32)
    #atac_mean = tf.convert_to_tensor(ys.atac_reducer.mean_,dtype=tf.float32)
    #atac_comp = tf.convert_to_tensor(ys.atac_reducer.components_,dtype=tf.float32)
